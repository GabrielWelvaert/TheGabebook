...
const NotificationModel = {
    async createNotification(link, datetime, senderId, recipientId, notificationUUID, text, subjectUUID){
        ...
        const [result] = await db.query(query, [link, datetime, senderId, recipientId,notificationUUID,text,false,subjectUUID]);
    ...
    async cullNotifications(recipientId){
        ...
        if (result[0].count >= maxNumberOfNotifications){
            const deleteQuery = `
                ...
                LIMIT 5;`;
            await db.query(deleteQuery, [recipientId]);
        }
    },
    async getTimeOfLastNotificationForSubjectBetweenTwoUsers(senderId, recipientId, subjectUUID, datetime){
        ...
        const query = ` 
            ...
            LIMIT 1;
        `;
        const [rows] = await db.query(query, [senderId, recipientId, subjectUUID]);
    ...
    async seen(notificationUUID){
        ...
        const [rows] = await db.query(query, [notificationUUID]);
    ...
    async getLastNotification(userId, otherId){
        ...
        const [rows] = await db.query(query, [userId, otherId]);
        return rows[0] ? rows[0] : undefined;
    }
}
