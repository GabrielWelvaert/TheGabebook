...
const CommentModel = {
    async getCommentIdFromUUID(commentUUID){
        ...
        const [rows] = await db.query(query, [commentUUID]);
    ...
    async submitComment(data){
        ...
        if(result.insertId){
            const [rows] = await db.query(`SELECT BIN_TO_UUID(commentUUID, true) as commentUUID, text, datetime FROM comment WHERE commentId = ?`, [result.insertId]);
    ...
    async commentExists(commentId){
        ...
        const [rows, fields] = await db.query(query, [commentId]);
        return rows[0] ? rows[0] : undefined;       
    },
