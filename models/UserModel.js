...
const UserModel = {
    ...
    async cullUsersIfThereAreTooMany(){
        ...
        let [rows] = await db.query(countQuery);
        count = rows[0].count;
        if(count >= 25){
            const deleteQuery = `DELETE FROM user WHERE confirmed = 0;`;
            await db.query(deleteQuery);
            [rows] = await db.query(countQuery);
            count = rows[0].count;
        }
        return count;
    },
    ...
    async resetPassword(userId, password){
        ...
        const [rows] = await db.query(query, [password, userId]);
        return rows.affectedRows > 0;
    },
    ...
    async searchUser(firstName, lastName){
        const query = `SELECT BIN_TO_UUID(userUUID, true) as userUUID, firstName, lastName, profilePic 
                        ...
                        OR LOWER(lastName) LIKE LOWER(?)
                        LIMIT 25;`;
        const [rows] = await db.query(query, ["%"+firstName+"%", "%"+lastName+"%","%"+firstName+"%"]);
